...
- 事件类型、schema、别名和 provider 注册。
- 轮询、定时、签名 Webhook 和可信内部发布接入。
- Observation 规范化、Trigger 判定和 DomainEvent 发布。
- 事件与 delivery 幂等持久化。
- delivery 领取租约、成功或失败回写、有限重试和 dead 状态。
- 内部 API、健康检查、指标、审计和 provider 故障隔离。
...
- 将某个 provider 的领域字段泄漏到核心鉴权、订阅、调度或 outbox。
...
第一版以纯文本为主。Plugin 负责验证渠道限制和安全分段；EventServer 不在 provider 中
硬编码 QQ API 参数。
...
```text
调度或接收原始数据
  -> provider 校验并生成 Observation
  -> 保存 checkpoint 和必要观察状态
  -> trigger 产生 DomainEvent
...
- 首次观察默认只建立基线；只有 provider metadata 明确允许时才发布初始事件。
...
- 不暴露 provider 私有配置、数据库内部字段或原始数据源响应。
...
新增 provider 必须实现 `metadata`、`validate_config`、`normalize`、`evaluate`、`render`
和 `health` 所需能力，并按接入模式声明 capability。

约束：

- provider 只能依赖 `eventserver.core` 通用契约。
- provider 不得读写用户、订阅或 delivery 表。
- provider 不得直接发送消息或调用 LLM。
- 外部 HTTP 客户端必须设置超时、响应大小上限、User-Agent、有限重试和 SSRF 防护。
- provider 返回的数据必须通过 schema 校验后才能进入事件存储。
- provider 异常只能影响自身调度。
- 每个 provider 具有独立并发限制、速率限制、熔断状态和最近成功时间。
- provider 配置只能来自管理员配置或受控部署。
...
- 日志包含 `request_id`、`provider_key`、`event_id` 或 `delivery_id`。
- OpenID、pairing code、租约 Token 和外部 payload 必须脱敏。
- 指标至少覆盖 provider 健康、调度延迟、事件数、delivery 积压、租约过期、retry 和 dead。
- `/health` 表示进程存活；`/ready` 表示依赖就绪。
- 非关键 provider 故障不应使整个服务失去存活状态。
- 对异常积压、连续 provider 失败、Webhook 验签失败和 dead delivery 建立告警。
...
- 内部 API 不直接暴露公网。
- 普通用户不能配置 URL、脚本、SQL、模板表达式或 provider 代码。
- 跨项目接口变更必须先更新版本化契约和双方契约测试。
- 不兼容的 provider、模板或事件结构变更必须提升 schema 版本。

## 12. 合规要求

实现和变更必须覆盖：

- provider 首次基线、无变化、变化、超时和无效响应。
...
- 敏感信息不进入日志。
- 新 provider 不修改权限、订阅、outbox 或 Plugin 核心代码。
- 迁移、备份、恢复、回退和 provider 下线流程具备可执行文档。
