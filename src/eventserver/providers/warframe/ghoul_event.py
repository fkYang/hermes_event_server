...
PROVIDER_KEY = "warframe.ghoul_event"
EVENT_KEY = "warframe.ghoul.started"


class GhoulEventProvider:
    ...
    def normalize(self, raw: dict[str, Any]) -> Observation:
        ...
        return observation(
            ...
            state={"active": require(raw, "active", bool)},
        )

    def evaluate(self, previous: Observation | None, current: Observation) -> list[DomainEvent]:
        if (
            previous is None
            or previous.state.get("active") is True
            or current.state["active"] is not True
        ):
            return []
        return [
            build_event(
                ...
                subject={"type": "world_event", "id": "ghouls"},
    ...
    def render(self, event: DomainEvent, locale: str = "zh-CN") -> DeliveryMessage:
        text = "尸鬼活动已出现" if locale.startswith("zh") else "The Ghoul event has started"
        return DeliveryMessage(
            title="事件提醒" if locale.startswith("zh") else "Event alert",
            text=text,
            template_key="warframe.ghoul.started.default",
            template_version=1,
        )
